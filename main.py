...
def test_casenet(model,testset):
    ...
    for i,(x,coord) in enumerate(data_loader):
        ...
        #print(out.data.cpu().numpy(),"nodulePred")
        predlist.append(casePred.data.cpu().numpy())
        outlist.append(out.data.cpu().numpy().reshape(5))
        #print([i,data_loader.dataset.split[i,1],casePred.data.cpu().numpy()])
    predlist = np.concatenate(predlist)
    return predlist,outlist
config2['bboxpath'] = bbox_result_path
config2['datadir'] = prep_result_path
#all file
#get all files name
pre_test_set = '../data/prep_test_set/'
bbox_set = './bbox_result/'
patient_predictions_csv = []
filelist = [f for f in os.listdir(datapath)]
imglist = [("%s_clean.npy" % f )for f in filelist]
pbblist = [("%s_pbb.npy" % f) for f in filelist]
#print(filelist,imglist,pbblist)
dataset = DataBowl3Classifier(testsplit, config2, phase = 'test')
predlist, outlist = test_casenet(casenet,dataset)
print( outlist,'predlist out')
#for every patience
for i in range(len(filelist)):
    pbb = np.load(bbox_set + pbblist[i])
    pbb = pbb[pbb[:,0]>-1]
    pbb = nms(pbb,0.05)
    for j in range(pbb.shape[0]):
         box = pbb[j].astype("int")[1:]
         centerX = box[2] 
         centerY = box[1]
         centerZ = box[0]
         radius = box[3]
         print( [filelist[i], centerX, centerY, centerZ, radius, outlist[i][j]])
         patient_predictions_csv_line = [filelist[i], centerX, centerY, centerZ, radius, outlist[i][j]]
...
#get out value 
df1 = pandas.DataFrame(patient_predictions_csv, columns=["patient_name", "coord_x", "coord_y", "coord_z", "diameter", "nodule_chance"])
df = pandas.DataFrame({'id':testsplit, 'cancer':predlist})
df.to_csv(filename,index=False)
df1.to_csv("finalout_pout.csv",index=False)
